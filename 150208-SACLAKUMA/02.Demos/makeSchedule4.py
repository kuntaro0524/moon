...
def makeSchedule():
	dire="/isilon/users/target/target/Staff/2014B/150209/00.CcOTest/"
	template="./kunio.sch"
	lines=open(template,"r")
	
	for line in lines:
		if line.rfind("DIRECTORY")!=-1:
			line=line.replace("DIRECTORY",param)
...
step_phi=float(sys.argv[4]) # interval angle for each irradiation point
...
# Initialization
serial_offset=0
outdir="/isilon/users/target/target/Staff/2014B"
wave=1.0
camelen=200.0
prefix=po1129
