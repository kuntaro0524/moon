...
class ReflWidthStillLaue:

	def __init__(self,amatfile,divv,divh,mosaic,dispersion,wave_proc=1.24,pol=1.0):
		...
		self.pol=pol # polarization factor 
	...
	def setHKL(self,hkl,phistart):
		...
		# For still reflection: startphi should be applied
		self.phimid=phistart
		phimid_matr=self.makeRotMat(self.phimid)

		# Amat x Rotation matrix  (still includes wavelength term)
		...
		# Calculate DPS XYZ coordinate without wavelength term
		# unit of xyz1 = (angstrome)^-1
		self.xyz1=dot(self.amat_midphi,float_hkl)/self.wl
		...
		## Calculating d* value without wavelength term
	...
	# ====================
	# pol: polarization factor ~0.95 for SR beam
	# ====================
	def calcLP(self,pol):
		self.dstar2=self.dstar*self.dstar
		self.dst4=self.dstar2*self.dstar2*0.25

		# Calculation of Lorentz factor from MOSFLM (SUBROUTINE LPCOR)
		zeta=self.xyz1[2]
		# DSTSQ
		snsq=0.25*self.dstar2
		cs2th=1.0-2.0*snsq
		cc2th=cs2th*cs2th
		ss2th=1.0-cc2th
		zetasq=zeta*zeta
		...
		# Polarisation factor
		fpol=(1.0+cc2th)*0.5-0.5*pol*ss2th*cos2ro
			
		# LP factor
		self.lpfac=lorinv/fpol
	...
	def calcRspot(self):
		...
		# Energy dispersion? NO 
		# 130604: memo: this is 0.0 for the default MOSFLM setting
		delcor=0.0000
		...
		self.divergence=sqrt((pow(esyn_h,2.0)+pow(esyn_v,2.0))/(yms+z1*z1))
		...
		self.rspot=(self.divergence+mosaic)*sqrt(self.dstar2-self.dst4) \
			+0.25*self.dispersion*self.dstar2+mos_term
		#print "DSTAR/RSPOT/MOS_TERM=",self.dstar,self.rspot-mos_term,mos_term
		#-print "DIVERG/ETA/DSTAR2/RSPOT=",self.divergence,mosaic,self.dstar2,self.rspot
