...
class RLP:
	...
	def setHKL(self,hkl,phi):
		...
		# Rotation matrix with phi ((startphi+endphi)/2.0 in MOSFLM)
		phi_matr=self.makeRotMat(phi)

		# Amat x Rotation matrix (phi)
		self.amat_phi=dot(phi_matr,self.amat)
		if self.isSetMisset:
	...
	def calcQ(self):
		q=math.pi-2.0*math.acos(self.del1/2.0/self.dstar)
		return q
