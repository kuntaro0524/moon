...
class ReflWidthBothEdge:
	...
	def setHKL(self,hkl,phistart):
		...
		if self.isSetMisset:
			self.amat_startphi=dot(self.misset,self.amat_startphi)

		# Amat x Rotation matrix (@end phi)
		self.amat_endphi=dot(phiend_matr,self.amat)
		if self.isSetMisset:
			self.amat_endphi=dot(self.misset,self.amat_endphi)
	...
	def calcDELEPS(self):
		...
		if self.cuspflag<0:
			...
			# del1 > 0.0 and del2 < 0.0 
			# or
			# del1 < 0.0 and del2 > 0.0
			### IMPORTANT ####

			if self.del1*self.del2<0.0:
				self.isFull=True
			else:
				self.isFull=False
		elif dist1<0.0:
