...
class ReflWidthBothEdge:
	...
	def __init__(self,amatfile,divv,divh,mosaic,dispersion,oscstep):
		...
		self.oscstep=oscstep 	# UNIT: degree
	...
	def setHKL(self,hkl,phistart):
		...
		self.amat_startphi=dot(phistart_matr,self.amat)

		# Amat x Rotation matrix (@end phi)
		self.amat_endphi=dot(phiend_matr,self.amat)

		#print "==== STARTPHI ====="
		#print self.amat_startphi
		#print "==== ENDPHI ====="
		#print self.amat_endphi
	...
	def calcDELEPS(self):
		...
		#print "DEL1=%12.9f DEL2=%12.9f"%(self.del1,self.del2)
	...
	def calcPartiality(self):
		...
		if self.deleps1<0.00001 and self.deleps2<0.00001:
			self.pcalc=0.0
			return self.pcalc

		elif self.deleps2<0.00001 :
			self.pcalc=0.5*(1.0-cos(self.deleps1*pi))
			return self.pcalc

		elif self.deleps1<0.00001:
			self.pcalc=0.5*(1.0-cos(self.deleps2*pi))
			return self.pcalc

		else:
			p1=0.5*(1.0-cos(self.deleps1*pi))
			p2=0.5*(1.0-cos(self.deleps2*pi))
			self.pcalc=p1-(1.0-p2)
			return self.pcalc
