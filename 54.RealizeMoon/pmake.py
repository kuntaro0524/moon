...
class ProfileMaker:
	...
	def averageProfile(self,dstar_thresh):
		...
		for neighbor_bunch,intensity_array in zip(self.Neighbors,self.reflist_i):
			...
			sumSig=0.0
			num=0
			# Estimating the averaged Gauss profile from near reflections
			# ref_i: neighbor reflection list (top index of each independent one)
			for ref_i in neighbor_bunch:
				A,mean,sigma=self.Profile[ref_i]

				# Averaging SIGMA and MEAN value among useful reflections
				# If A == -9999 or A == -1111: Gaussian fitting was failed
				# Then, these reflections are rejected here
...
if __name__ == "__main__":
	...
	# storing information into the common array
	h.prepInfo(matfile,phistart,deltaphi)

	# Making Profile lists
	h.bunch()

	# RLP coordinates and Q and Lorentz factor estimation
	h.calcXYZQL(mx,my,mz)

	# Gaussian fitting of each reflection profile
